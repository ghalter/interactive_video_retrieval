...
def create_hilbert_transform(s, as_float = False):
    ...
    if as_float:
        ...
        colors[:, 1] = colors[:, 1] - 128
        colors[:, 2] = colors[:, 2] - 128
        colors = np.array([colors, colors], dtype=np.float32)
        print(np.amin(colors), np.amax(colors))
    else:
